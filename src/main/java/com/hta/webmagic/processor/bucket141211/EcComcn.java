...
public class EcComcn implements PageProcessor, Runnable {
	private String seedUrl = "http://www.ec.com.cn/";
	private String siteNmae = "中国国际电子商务网";
	...
	public void process(Page page) {
		...
		if (page.getUrl()
				.toString()
				.matches(
						"http://[a-z]*.ec.com.cn/article/[a-z]*/\\d*/[0-9_]*.html")) {
...
//			showinfo.printresult(rs);
	...
	private static result getDocument(result rs, String html) {
		...
		Elements Title = doc.select("div.title").select("h1");
		String title = Title.text();
		rs.setTitle(title);

		Elements Content = doc.select("div.article");
	...
	private static Date findTime(String html) {
		...
		Elements trsid = doc.select("div.title").select("p");
		String time = regex.findTime(trsid.html(),
				"\\d{4}-\\d*-\\d* \\d*:\\d*:\\d*");
	...
	public void run() {
		...
		Spider.create(new EcComcn()).addUrl(seedUrl)
	...
	public static void main(String[] args) {
		...
		localurl = pro.getProperty("LOCALURL");
		StopDeepStr = pro.getProperty("DEEP");
		Spider.create(new EcComcn()).addUrl("http://www.ec.com.cn")
