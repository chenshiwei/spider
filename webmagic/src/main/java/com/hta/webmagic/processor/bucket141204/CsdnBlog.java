...
public class CsdnBlog implements PageProcessor, Runnable {
  private String seedUrl = "http://blog.csdn.net/";
  private String siteNmae = "csdn博客";
  ...
  public void process(Page page) {
    ...
    if (page.getUrl().toString().matches("http://blog.csdn.net/.*/article/details/[^#]*")) {
  ...
  private static result getDocument(result rs, String html) {
    ...
    Elements Title = doc.select("span[class=link_title]");
    String title = Title.text();
    rs.setTitle(title);

    Elements Content = doc.select("div[class=article_content]");
    ...
    String time = doc.select("span[class=link_postdate]").text();
    if (time.length() > 10) {
      time = time.substring(0, 10);
    }
    System.out.println("time:" + time);
    rs.setTimeD(Timer.findDate(time));
    return rs;
  }
  ...
  public static void main(String[] args) {
    ...
    // .addPipeline(new mysqlPipeline())
        .addUrl("http://blog.csdn.net/u014304293/article/details/42058401").thread(5).run();
  }
  ...
  public void run() {
    ...
    localurl = pro.getProperty("LOCALURL");
    StopDeepStr = pro.getProperty("DEEP");
    Spider.create(new CsdnBlog()).addPipeline(new mysqlPipeline()).addUrl("http://blog.csdn.net/")
        .thread(5).run();
  }
